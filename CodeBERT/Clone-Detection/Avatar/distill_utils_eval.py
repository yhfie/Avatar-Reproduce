...
def distill(hyperparams_set, eval=True, surrogate=False):
    ...
    for hyperparams in hyperparams_set:
        ...
        if not eval:
            ...
            csv_file_exists = os.path.exists(result_csv_path)
            with open(result_csv_path, 'a', newline='') as csvfile: # 'a' for append mode
                fieldnames = ['name', 'task', 'acc', 'precision', 'recall', 'f1']
